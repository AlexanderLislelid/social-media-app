...
`,e}function R(e="button",t="primary"){const s=document.createElement("button"),a="px-3 py-2 text-sm rounded-lg",o={primary:"bg-indigo-500 hover:bg-indigo-600 text-white font-medium",danger:"bg-red-600 hover:bg-red-700 text-white font-medium",logout:" bg-slate-900 hover:bg-slate-950 border border-slate-600 text-white font-medium",close:"bg-slate-900 border border-slate-700  hover:border-indigo-500 hover:text-indigo-300",profile:"bg-slate-900 border border-slate-700  hover:border-indigo-500 hover:text-indigo-300 px-2 py-1"};return s.className=`${a} ${o[t]}`.trim(),s.textContent=e,s}function Q(){const e=document.getElementById("post-modal"),t=document.getElementById("modal-close");function s(){e.classList.add("hidden"),document.body.classList.remove("overflow-hidden"),e.classList.remove("flex"),document.getElementById("post-modal-content").innerHTML=""}document.addEventListener("keydown",a=>{a.key==="Escape"&&s()}),t.onclick=s}async function Y(e){const t=document.getElementById("post-modal"),s=document.getElementById("post-modal-content"),a=document.getElementById("btn-wrapper");a.innerHTML="",t.classList.remove("hidden"),t.classList.add("flex"),document.body.classList.add("overflow-hidden");const n=(await N(`social/posts/${e}?_author=true&_comments=true&_reactions=true`)).data,l=new Date(n.created).toLocaleString("no-NO",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div"),r=document.createElement("div"),m=document.createElement("div"),p=document.createElement("p"),x=document.createElement("h2"),f=document.createElement("p"),u=document.createElement("p");if(d.className="bg-slate-800 border border-slate-700 rounded-xl shadow-md p-4 w-full max-w-2xl mx-auto space-y-3 text-slate-100",r.className="flex justify-between items-center",m.className="flex items-center gap-2",p.className="font-semibold text-slate-100",f.className="text-xs text-slate-400",x.className="text-lg font-semibold text-slate-100",u.className="text-slate-300 leading-relaxed",p.textContent=n.author.name,f.textContent=l,x.textContent=n.title,u.textContent=n.body,n.author?.avatar?.url){const i=document.createElement("img");i.className="w-14 h-14 rounded-full object-cover border border-slate-700",i.src=n.author.avatar.url,i.alt=`${n.author.name}'s profile picture`,m.append(i)}if(n.media?.url){const i=document.createElement("img");i.className="w-full rounded-lg object-cover max-h-[500px] border border-slate-700",i.src=n.media.url,i.alt=n.media?.alt??"Post image",d.append(i)}const g=R("View Profile","primary");g.addEventListener("click",()=>{t.classList.add("hidden"),t.classList.remove("flex"),document.body.classList.remove("overflow-hidden"),window.location.hash=`#/user/${n.author.name}`}),a.append(g),r.append(m,f),d.append(r),d.append(x),d.append(u);const c=document.createElement("div");c.className="mt-6 space-y-2";const v=n.comments;if(v.length===0){const i=document.createElement("p");i.className="text-sm text-slate-400 border-t border-slate-700 pt-2 mt-4",i.textContent="No comments yet",c.append(i)}else v.forEach(i=>{const h=document.createElement("div"),b=document.createElement("div"),w=document.createElement("img"),E=document.createElement("p"),C=document.createElement("p");h.className="border-t border-slate-700 pt-2 mt-4",b.className="flex items-center gap-4 py-2",C.className="text-sm font-semibold text-slate-100",E.className="text-sm text-slate-300",C.textContent=i.author.name,E.textContent=i.body,w.className="w-8 h-8 rounded-full border border-slate-700",w.src=i.author.avatar.url,w.alt=`${i.author.name}'s profile picture`,b.append(w,C),h.append(b,E),c.append(h)});d.append(c),s.append(d)}function ee(e){const t=document.createElement("div"),s=document.createElement("h2"),a=document.createElement("p"),o=document.createElement("p"),n=document.createElement("div"),l=document.createElement("div"),d=document.createElement("div"),r=document.createElement("span"),m=document.createElement("div"),p=document.createElement("div"),x=document.createElement("p"),f=document.createElement("div"),u=document.createElement("p");if(t.style.cursor="pointer",t.className="post-card",o.className="text-xs text-slate-400",a.className="post-textarea bg-slate-700 text-slate-100 border border-slate-800 shadow-md",l.className="flex justify-between items-center text-slate-400",d.className="flex items-center gap-2",m.className="w-10 h-10 rounded-full overflow-hidden",x.className="text-sm text-slate-400 ml-2 mt-2",u.className="text-sm text-slate-400",p.className="flex justify-between",f.className="flex items-center gap-1 text-sm text-slate-400 ml-2 mt-2",e.media?.url){const c=document.createElement("img");c.src=e.media.url,c.alt="Post image",n.append(c)}if(e.author?.avatar?.url){const c=document.createElement("img");c.src=e.author.avatar.url,c.alt=`${e.author.name} avatar`,c.className="w-10 h-10 rounded-full",m.append(c)}const g=new Date(e.updated).toLocaleString("no-NO",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"});return s.textContent=e.title.charAt(0).toUpperCase()+e.title.slice(1),a.textContent=e.body,o.textContent=g,r.textContent=e.author?.name||"Unknown user",x.textContent=`${e._count.comments} Comments`,u.textContent=e._count.reactions,t.onclick=()=>Y(e.id),f.append(u,J()),p.append(x,f),d.append(m,r),l.append(d,o),t.append(l,s,n,a,p),t}async function te(){if(A()&&_())return`
    ...
    `;window.location.hash="#/login"}let y=1,S=!1,B="";async function k(e,t=""){const s=document.getElementById("posts"),a=document.getElementById("next-page-btn");if(!(!s||!a)){S=!0,a.textContent="Loading...",a.disabled=!0;try{let o=`social/posts?page=${e}&limit=10&_author=true&_count=true`;t&&(o=`social/posts/search?q=${encodeURIComponent(t)}&_author=true&_count=true`);const n=await N(o),l=n.data,d=n.meta;s.innerHTML="",l.forEach(r=>{const m=ee(r);s.append(m)}),d.isLastPage?a.style.display="none":(a.textContent="Next Page",a.disabled=!1)}catch(o){console.error("Failed to fetch posts:",o),alert(o)}finally{S=!1}}}function oe(){const e=document.getElementById("next-page-btn"),t=document.getElementById("prev-page-btn");e.onclick=()=>{S||(y++,k(y,B),window.scrollTo(0,0))},t.onclick=()=>{S||y<=1||(y--,k(y,B),window.scrollTo(0,0))}}function ne(){const e=document.getElementById("search-form"),t=document.getElementById("search");e.addEventListener("submit",s=>{s.preventDefault(),B=t.value.trim(),y=1,k(y,B),window.scrollTo(0,0)}),t.addEventListener("input",()=>{t.value===""&&(B="",y=1,k(y,B))})}function se(){return`
  ...
  `}function le(){const e=document.getElementById("loginForm"),t=document.getElementById("email"),s=document.getElementById("password"),a=document.getElementById("loginError");e.addEventListener("submit",async o=>{o.preventDefault(),a.classList.add("hidden"),a.textContent="";try{const n=await $("auth/login",{email:t.value.trim(),password:s.value});if(K(n.data.accessToken),localStorage.setItem("username",n.data.name),!_()){const l=await $("auth/create-api-key",{name:"Social-App"});z(l.data.key)}window.location.hash="#/"}catch(n){a.textContent=n.message,a.classList.remove("hidden")}})}function re(e){return e?/^[A-Za-z0-9_]+$/.test(e.trim()):!1}function de(e){if(e)return/^[^\s@]+@stud\.noroff\.no$/.test(e.trim())}function ce(e){return e?e.length>=8:!1}function ie(){return`
    <section class="px-4 mt-12">
      <div
        class="max-w-md mx-auto bg-slate-800 border border-slate-700 rounded-xl shadow p-6 space-y-6"
      >
  ...
  `}async function j(){const e=document.getElementById("logout"),t=document.getElementById("profile-page"),s=document.getElementById("posts");if(!A()){e.style.display="none",t.innerHTML="Please Log in to View your profile",s.innerHTML="";return}const a=localStorage.getItem("username"),o=await N(`social/profiles/${a}`),n=await N(`social/profiles/${a}/posts?_author=true&_count=true`),l=o.data,d=n.data;s.innerHTML="";const r=document.getElementById("profileWelcome"),m=document.getElementById("avatar"),p=document.getElementById("followers"),x=document.getElementById("following"),f=document.getElementById("number-of-posts");r.textContent=`Welcome back ${l.name}`,m.src=l.avatar.url,m.alt=`${l.name} avatar`,p.replaceChildren(),p.append(X(),` Followers: ${l._count.followers}`),x.replaceChildren(),x.append(F(),` Following: ${l._count.following}`),f.replaceChildren(),f.append(W(),` Posts: ${l._count.posts}`),d.forEach(u=>{const g=document.createElement("div"),c=document.createElement("h2"),v=document.createElement("p"),i=document.createElement("div"),h=document.createElement("button"),b=document.createElement("button"),w=document.createElement("dialog"),E=document.createElement("div"),C=document.createElement("label"),U=document.createElement("label"),P=document.createElement("input"),I=document.createElement("textarea"),O=document.createElement("div"),M=document.createElement("button"),T=document.createElement("button");if(C.textContent="Title",U.textContent="Body",c.textContent=u.title,v.textContent=u.body,h.textContent="Delete post",b.textContent="Update Post",M.textContent="Update",T.textContent="Close",g.className="w-full max-w-md bg-slate-800 border border-slate-700 rounded-xl shadow p-5 space-y-4",c.className="text-lg font-semibold text-slate-100",v.className="text-slate-300",i.className="flex gap-3 pt-2",h.className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium",b.className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium",w.className="bg-slate-900 text-slate-100 rounded-xl p-6 border border-slate-700 w-full max-w-md",E.className="flex flex-col gap-4",C.className="text-sm text-slate-300",U.className="text-sm text-slate-300",P.className="w-full p-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 ",I.className="w-full p-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 ",I.rows=4,O.className="flex justify-end gap-2 pt-2",M.className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium ",T.className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium ",g.append(c),u.media?.url){const L=document.createElement("img");L.src=u.media.url,L.alt=u.media.alt||"Post image",L.className="w-full rounded-lg border border-slate-700",g.append(L)}g.append(v),O.append(T,M),E.append(C,P,U,I,O),w.replaceChildren(E),g.append(w),i.append(h,b),g.append(i),s.append(g),h.addEventListener("click",async()=>{confirm("Are you sure you want to delete this post?")&&(await G(`social/posts/${u.id}`),j())}),b.addEventListener("click",()=>{P.value=u.title,I.value=u.body,w.showModal()}),M.addEventListener("click",async()=>{await D(`social/posts/${u.id}`,{title:P.value,body:I.value}),w.close(),j()}),T.addEventListener("click",()=>{w.close()})}),e.textContent="Logout",e.addEventListener("click",()=>{localStorage.removeItem("username"),Z(),window.location.hash="#/login"})}async function pe(){if(A()&&_())return`
      <form
        id="create-form"
  ...
  `}async function xe(e){const t=(await N(`social/profiles/${e}?_following=true&_followers=true&_posts=true`)).data,s=document.getElementById("avatar"),a=document.getElementById("profileWelcome"),o=document.getElementById("user-bio"),n=document.getElementById("followers-number"),l=document.getElementById("posts-number"),d=document.getElementById("btn-container"),r=R("Follow"),m=document.getElementById("posts-heading"),p=document.getElementById("user-email"),x=localStorage.getItem("username");let f=t.followers.some(c=>c.name===x);u(),d.append(r);function u(){f?r.textContent="Unfollow":r.textContent="Follow"}r.onclick=async()=>{try{f?(await D(`social/profiles/${t.name}/unfollow`),f=!1):(await D(`social/profiles/${t.name}/follow`),f=!0),u()}catch(c){alert(c.message)}},s.src=t.avatar.url,a.textContent=`${t.name}'s Profile`,p.textContent=`@${t.email}`,t.posts.length>0?m.textContent=`${t.name}'s Posts`:m.textContent=`${t.name} has not made any posts yet`,t.bio===null?o.textContent="This user has not updated their bio yet":o.textContent=t.bio,d.append(r),n.append(F()),n.append(t.followers.length),l.append(W()),l.append(t.posts.length);const g=document.getElementById("posts");t.posts.forEach(c=>{const v=document.createElement("div"),i=document.createElement("h2"),h=document.createElement("p"),b=document.createElement("img");i.textContent=c.title,h.textContent=c.body,b.src=c.media?.url||"",b.alt=c.media?.alt||"",i.className="text-slate-100 text-lg mb-2",h.className="text-slate-400 text-sm mb-4",v.className="bg-slate-800 p-4 rounded-lg shadow-md",b.className="rounded-lg",v.append(i,h,b),g.append(v)})}function ge(){const e={"#/":te,"#/login":ae,"#/register":ie,"#/profile":ue,"#/create":pe,"#/user":V};async function t(){const s=window.location.hash||"#/",[a,o,n]=s.split("/");if(o==="user"&&n){document.getElementById("app").innerHTML=V(),await xe(decodeURIComponent(n));return}const l=e[s]||se;document.getElementById("app").innerHTML=await l(),s==="#/login"&&le(),s==="#/profile"&&j(),s==="#/register"&&me(),s==="#/create"&&fe(),s==="#/"&&(oe(),ne(),Q(),k(1,""))}window.addEventListener("hashchange",t),window.addEventListener("load",t)}window.addEventListener("DOMContentLoaded",()=>{ge()});
