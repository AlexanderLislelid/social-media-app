...
`,e}function R(e="button",t="primary"){const s=document.createElement("button"),a="px-3 py-2 text-sm rounded-lg",o={primary:"bg-indigo-500 hover:bg-indigo-600 text-white font-medium",danger:"bg-red-600 hover:bg-red-700 text-white font-medium",logout:" bg-slate-900 hover:bg-slate-950 border border-slate-600 text-white font-medium",close:"bg-slate-900 border border-slate-700  hover:border-indigo-500 hover:text-indigo-300",profile:"bg-slate-900 border border-slate-700  hover:border-indigo-500 hover:text-indigo-300 px-2 py-1"};return s.className=`${a} ${o[t]}`.trim(),s.textContent=e,s}function Q(){const e=document.getElementById("post-modal"),t=document.getElementById("modal-close");function s(){e.classList.add("hidden"),document.body.classList.remove("overflow-hidden"),e.classList.remove("flex"),document.getElementById("post-modal-content").innerHTML=""}document.addEventListener("keydown",a=>{a.key==="Escape"&&s()}),t.onclick=s}async function Y(e){const t=document.getElementById("post-modal"),s=document.getElementById("post-modal-content"),a=document.getElementById("btn-wrapper");a.innerHTML="",t.classList.remove("hidden"),t.classList.add("flex"),document.body.classList.add("overflow-hidden");const n=(await L(`social/posts/${e}?_author=true&_comments=true&_reactions=true`)).data,l=new Date(n.created).toLocaleString("no-NO",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div"),r=document.createElement("div"),m=document.createElement("div"),p=document.createElement("p"),x=document.createElement("h2"),f=document.createElement("p"),u=document.createElement("p");if(d.className="bg-slate-800 border border-slate-700 rounded-xl shadow-md p-4 w-full max-w-2xl mx-auto space-y-3 text-slate-100",r.className="flex justify-between items-center",m.className="flex items-center gap-2",p.className="font-semibold text-slate-100",f.className="text-xs text-slate-400",x.className="text-lg font-semibold text-slate-100",u.className="text-slate-300 leading-relaxed",p.textContent=n.author.name,f.textContent=l,x.textContent=n.title,u.textContent=n.body,n.author?.avatar?.url){const i=document.createElement("img");i.className="w-14 h-14 rounded-full object-cover border border-slate-700",i.src=n.author.avatar.url,i.alt=`${n.author.name}'s profile picture`,m.append(i)}if(n.media?.url){const i=document.createElement("img");i.className="w-full rounded-lg object-cover max-h-[500px] border border-slate-700",i.src=n.media.url,i.alt=n.media?.alt??"Post image",d.append(i)}const g=R("View Profile","primary");g.addEventListener("click",()=>{t.classList.add("hidden"),t.classList.remove("flex"),document.body.classList.remove("overflow-hidden"),window.location.hash=`#/user/${n.author.name}`}),a.append(g),r.append(m,f),d.append(r),d.append(x),d.append(u);const c=document.createElement("div");c.className="mt-6 space-y-2";const v=n.comments;if(v.length===0){const i=document.createElement("p");i.className="text-sm text-slate-400 border-t border-slate-700 pt-2 mt-4",i.textContent="No comments yet",c.append(i)}else v.forEach(i=>{const h=document.createElement("div"),b=document.createElement("div"),w=document.createElement("img"),y=document.createElement("p"),E=document.createElement("p");h.className="border-t border-slate-700 pt-2 mt-4",b.className="flex items-center gap-4 py-2",E.className="text-sm font-semibold text-slate-100",y.className="text-sm text-slate-300",E.textContent=i.author.name,y.textContent=i.body,w.className="w-8 h-8 rounded-full border border-slate-700",w.src=i.author.avatar.url,w.alt=`${i.author.name}'s profile picture`,b.append(w,E),h.append(b,y),c.append(h)});d.append(c),s.append(d)}function ee(e){const t=document.createElement("div"),s=document.createElement("h2"),a=document.createElement("p"),o=document.createElement("p"),n=document.createElement("div"),l=document.createElement("div"),d=document.createElement("div"),r=document.createElement("span"),m=document.createElement("div"),p=document.createElement("div"),x=document.createElement("p"),f=document.createElement("div"),u=document.createElement("p");if(t.style.cursor="pointer",t.className="post-card",o.className="text-xs text-slate-400",a.className="post-textarea bg-slate-700 text-slate-100 border border-slate-800 shadow-md",l.className="flex justify-between items-center text-slate-400",d.className="flex items-center gap-2",m.className="w-10 h-10 rounded-full overflow-hidden",x.className="text-sm text-slate-400 ml-2 mt-2",u.className="text-sm text-slate-400",p.className="flex justify-between",f.className="flex items-center gap-1 text-sm text-slate-400 ml-2 mt-2",e.media?.url){const c=document.createElement("img");c.src=e.media.url,c.alt="Post image",n.append(c)}if(e.author?.avatar?.url){const c=document.createElement("img");c.src=e.author.avatar.url,c.alt=`${e.author.name} avatar`,c.className="w-10 h-10 rounded-full",m.append(c)}const g=new Date(e.updated).toLocaleString("no-NO",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"});return s.textContent=e.title.charAt(0).toUpperCase()+e.title.slice(1),a.textContent=e.body,o.textContent=g,r.textContent=e.author?.name||"Unknown user",x.textContent=`${e._count.comments} Comments`,u.textContent=e._count.reactions,t.onclick=()=>Y(e.id),f.append(u,J()),p.append(x,f),d.append(m,r),l.append(d,o),t.append(l,s,n,a,p),t}async function te(){if(A()&&_())return`
    ...
    `;window.location.hash="#/login"}let C=1,T=!1,$="";async function S(e,t=""){const s=document.getElementById("posts"),a=document.getElementById("next-page-btn");if(!(!s||!a)){T=!0,a.textContent="Loading...",a.disabled=!0;try{let o=`social/posts?page=${e}&limit=10&_author=true&_count=true`;t&&(o=`social/posts/search?q=${encodeURIComponent(t)}&_author=true&_count=true`);const n=await L(o),l=n.data,d=n.meta;s.innerHTML="",l.forEach(r=>{const m=ee(r);s.append(m)}),d.isLastPage?a.style.display="none":(a.textContent="Next Page",a.disabled=!1)}catch(o){console.error("Failed to fetch posts:",o),alert(o)}finally{T=!1}}}function oe(){const e=document.getElementById("next-page-btn"),t=document.getElementById("prev-page-btn");e.onclick=()=>{T||(C++,S(C,$),window.scrollTo(0,0))},t.onclick=()=>{T||C<=1||(C--,S(C,$),window.scrollTo(0,0))}}function ne(){const e=document.getElementById("search-form"),t=document.getElementById("search");e.addEventListener("submit",s=>{s.preventDefault(),$=t.value.trim(),C=1,S(C,$),window.scrollTo(0,0)})}function se(){return`
  ...
  `}async function j(){const e=document.getElementById("logout"),t=document.getElementById("profile-page"),s=document.getElementById("posts");if(!A()){e.style.display="none",t.innerHTML="Please Log in to View your profile",s.innerHTML="";return}const a=localStorage.getItem("username"),o=await L(`social/profiles/${a}`),n=await L(`social/profiles/${a}/posts?_author=true&_count=true`),l=o.data,d=n.data;s.innerHTML="";const r=document.getElementById("profileWelcome"),m=document.getElementById("avatar"),p=document.getElementById("followers"),x=document.getElementById("following"),f=document.getElementById("number-of-posts");r.textContent=`Welcome back ${l.name}`,m.src=l.avatar.url,m.alt=`${l.name} avatar`,p.replaceChildren(),p.append(X(),` Followers: ${l._count.followers}`),x.replaceChildren(),x.append(F(),` Following: ${l._count.following}`),f.replaceChildren(),f.append(W(),` Posts: ${l._count.posts}`),d.forEach(u=>{const g=document.createElement("div"),c=document.createElement("h2"),v=document.createElement("p"),i=document.createElement("div"),h=document.createElement("button"),b=document.createElement("button"),w=document.createElement("dialog"),y=document.createElement("div"),E=document.createElement("label"),U=document.createElement("label"),N=document.createElement("input"),B=document.createElement("textarea"),O=document.createElement("div"),k=document.createElement("button"),P=document.createElement("button");if(E.textContent="Title",U.textContent="Body",c.textContent=u.title,v.textContent=u.body,h.textContent="Delete post",b.textContent="Update Post",k.textContent="Update",P.textContent="Close",g.className="w-full max-w-md bg-slate-800 border border-slate-700 rounded-xl shadow p-5 space-y-4",c.className="text-lg font-semibold text-slate-100",v.className="text-slate-300",i.className="flex gap-3 pt-2",h.className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium",b.className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium",w.className="bg-slate-900 text-slate-100 rounded-xl p-6 border border-slate-700 w-full max-w-md",y.className="flex flex-col gap-4",E.className="text-sm text-slate-300",U.className="text-sm text-slate-300",N.className="w-full p-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 ",B.className="w-full p-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 ",B.rows=4,O.className="flex justify-end gap-2 pt-2",k.className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium ",P.className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium ",g.append(c),u.media?.url){const I=document.createElement("img");I.src=u.media.url,I.alt=u.media.alt||"Post image",I.className="w-full rounded-lg border border-slate-700",g.append(I)}g.append(v),O.append(P,k),y.append(E,N,U,B,O),w.replaceChildren(y),g.append(w),i.append(h,b),g.append(i),s.append(g),h.addEventListener("click",async()=>{confirm("Are you sure you want to delete this post?")&&(await G(`social/posts/${u.id}`),j())}),b.addEventListener("click",()=>{N.value=u.title,B.value=u.body,w.showModal()}),k.addEventListener("click",async()=>{await D(`social/posts/${u.id}`,{title:N.value,body:B.value}),w.close(),j()}),P.addEventListener("click",()=>{w.close()})}),e.textContent="Logout",e.addEventListener("click",()=>{localStorage.removeItem("username"),Z(),window.location.hash="#/login"})}async function pe(){if(A()&&_())return`
      <form
        id="create-form"
